...
**模型-only 部署不需要任何 B2 环境变量**。  
说明：如果不提供 `eicu_mimic_lab_time.csv`，则与“药物组合数据库分析”相关的接口将不可用，但预测/预警接口仍可用。
...
**问题**: 数据文件缺失（仅影响药物组合分析功能）
- **解决**: 这是预期行为（模型-only 部署不使用数据文件）
- **解决**: 如需启用药物组合分析，再额外提供 `eicu_mimic_lab_time.csv`
...
3. **数据文件**: 模型-only 部署不需要数据文件；如需启用药物组合分析功能，再补充数据文件。

4. **环境变量**: 如未来使用外部数据源/密钥，请用 Render Dashboard 环境变量管理，不要提交到代码仓库。
